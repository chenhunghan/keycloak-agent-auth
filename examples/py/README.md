# Agent Auth Protocol — Python example

A minimal end-to-end walk-through of the AAP roles, in Python, against the Keycloak instance `docker compose up` spins up.
...
|------|------|--------------|
| `src/aap_demo/client.py` | **Client** (AAP §1.5) | Holds the Ed25519 host keypair. Mints `host+jwt` (§4.2) for register / status / revoke / introspect; mints `agent+jwt` (§4.3) for `/capability/execute` and introspect target. Only process speaking HTTPS to Keycloak. |
| `src/aap_demo/agent.py` | **Agent** (AAP §2.1) | Stub for the AI reasoning loop. Holds no keys. Forwards tool invocations through the Client. |
| `src/aap_demo/resource_server.py` | **Resource Server** core | Minimal `http.server`-based listener hosting the capability's business logic. |
| `src/aap_demo/rs_standalone.py` | RS entrypoint | Boots the resource server on port 3000 inside docker compose. |
| `src/aap_demo/admin.py` | Admin plumbing | Gets an admin OIDC token, `POST`s a capability definition, approves grants via the admin API. |
| `src/aap_demo/demo.py` | Wiring | Walks both journeys. |

## Run it

**Prereqs**: Python 3.12+. The demo brings up its own stack (Keycloak + extension + resource server) via its own compose file:
...
Same scope as `examples/js`: gateway-mode execution only, no host-key persistence, no device-flow approval (admin path used for delegated journey), no key rotation. See the JS README for the rationale.
